...
const helpKnowledgeBase = [
  {
    ...
    answer:
      "To login as landlord, enter phone + password then verify OTP. To login as tenant, use the tenant code generated by the landlord (Dashboard → Generate Tenant Code).",
  },
  {
...
const renderLandlordLogin = () => {
  ...
  document.getElementById("landlordLoginForm").onsubmit = async (e) => {
    ...
    const msg = res.simulatedOtp ? `OTP (simulated): ${res.simulatedOtp}` : "OTP sent (check your phone).";
...
const renderTenantTicket = (type) => {
    ...
    <section class="screen">
      ...
      <div class="card house-content tenant-auth-card">
        ...
        <form id="tenantTicketForm">
          <label>
            Contact number
            <input name="contactNumber" placeholder="+2547XXXXXXXX" required />
          </label>
          <label>
            House name
  ...
  document.getElementById("tenantTicketForm").onsubmit = async (e) => {
    ...
    const payload = {
      type,
      contactNumber: safeText(formData.get("contactNumber")),
      ...
      tenantPhone: state.tenantProfile?.phone || null,
...
const renderLandlordDashboard = () => {
  ...
  const profileCard = `
    <aside class="dashboard-sidebar">
      <div class="card house-content profile-card">
        ...
        <p class="muted">${safeText(profile.phone) || "—"}</p>
      ...
      <div class="card house-content">
        ...
        <button id="generateCodeBtn" class="ghost-btn" type="button">Generate Tenant Code</button>
  ...
  `;
    ...
    document.getElementById("generateCodeBtn").addEventListener("click", async () => {
      const tenantPhone = prompt("Tenant phone number (+254...)");
      if (!tenantPhone) return;
      const propertyId = prompt("Property ID (optional) - paste the property id") || null;
      const res = await apiRequest("/landlord/tenant-codes", "POST", { tenantPhone, propertyId });
      if (!res) return;
      showStatusToast("Tenant Code: " + res.code, "success");
    });
    ...
    .map((n) => {
      ...
      return `
        <article class="notification-item">
          ...
          <p class="muted tiny">Contact: ${safeText(n.contactNumber)}</p>
        </article>
      `;
    })
    .join("");
...
const refreshLandlordNotifications = async () => {
  ...
  document.getElementById("houseForm").onsubmit = async (e) => {
    ...
    const payload = {
      ...
      ncaProjectId: form[4].value,
      location: form[5].value,
      fullAddress: form[6].value,
      photos,
    };
