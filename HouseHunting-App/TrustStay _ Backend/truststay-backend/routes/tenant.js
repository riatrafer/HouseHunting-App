...
router.post("/tickets", requireAuth(["tenant"]), async (req, res) => {
  try {
    ...
    const ticket = {
      ...
      contactEmail: (payload.contactEmail ?? "").toString().trim(),
      ...
      tenantEmail: (payload.tenantEmail ?? "").toString().trim() || null,
      tenantId: req.auth.userId,
      createdAt: new Date(),
      status: "new",
    };

    if (!ticket.contactEmail || !ticket.houseName || !ticket.roomNumber || !ticket.details) {
      return res.status(400).json({ message: "contactEmail, houseName, roomNumber, and details are required" });
    }
